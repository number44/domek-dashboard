...
const SectionA = ({}: PropsI) => {
	...
	const onSubmit: SubmitHandler<FormI> = (data) => {
		if (confirm('zapisać zmiany ?')) {
			setLoading(true);
			axios.post('/sections/1', { title: data.title, content: editor?.getHTML(), picture_1: state.imgId1, picture_2: state.imgId2, picture_3: state.imgId3, picture_4: state.imgId4 }).then((data) => {
				setLoading(false);
			});
		}
	};
			...
			{isOpen && (
				<Dialog className="fixed z-30 inset-0 overflow-y-auto flex-center" open={isOpen} onClose={() => setIsOpen(false)}>
					...
					<Dialog.Description as="div" className="  relative  py-2 p-12 z-20">
						<div className="mt- mt-48 grid grid-cols-2 h-screen sm:grid-cols-4 gap-3">
							{data?.map((pic) => (
