...
import { fetchPictures, fetchSlide } from '../../utils/fetching';
...
const AddSlide = ({}: PropsI) => {
	...
	useEffect(() => {
		axios.get(`/slides/${newId}`).then(({ data }) => {
			console.log('data :', data.data);
			setValue('header', data.data.header);
			setValue('description', data.data.description);
			setImgUrl(data.data.picture.picture_lg);
			setImgId(data.data.picture.picture_id);
		});
	}, []);
	...
	const mutation = useMutation(
		(newData: FormI) => {
			return axios.post(`/slides/${params.id}`, { header: newData.header, description: newData.description, picture_id: imgId });
		},
		{
			onSuccess: () => {
				router('/slides');
					...
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
						{data?.map((pic) => (
							<div key={pic.id} onClick={() => handler(pic.id, pic.picture_lg)} className="aspect-[12/8]">
								<img className="object-cover" src={pic.picture_md} alt="" />
							</div>
						))}
					</div>
				</Dialog.Description>
				<button onClick={() => setIsOpen(false)} className=" outline-0 rounded-full  text-gray-800 text-4xl  dark:text-gray-200 absolute w-8 h-8 flex-center right-8 top-8 hover:rotate-180 transition-transform duration-300 cursor-pointer z-10 ">
					<VscChromeClose />
				</button>
			</Dialog>
