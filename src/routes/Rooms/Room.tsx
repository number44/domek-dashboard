...
const Room = ({}: PropsI) => {
	...
	const onSubmit: SubmitHandler<RoomI> = (data) => {
		AboutPlRef.current?.uploadEditor();
		AboutEnRef.current?.uploadEditor();
		if (confirm('Zapisać zmiany ?')) mutation.mutate(data);
	};
	...
	const handleDelete = () => {
		if (confirm('Na pewno usunąć ??')) mutationDelete.mutate(newId);
	};
	...
	const uploadFromPepe = () => {
		...
		axios.get('/pep').then((res) => {
			...
			const sp = propertyArr[0].room[0].room_id;
			const result: any[] = [];
			propertyArr.forEach((el: any) => {
				const elem = el.room.find((r: any) => parseInt(r.room_id) == son_id);
				if (elem) {
					result.push(elem);
				}
			});
	...
	return (
		<>
			<div className="flex justify-end">
				<div onClick={handleDelete} className="cursor-pointer bg-white rounded dark:bg-slate-800 shadow-sm p-3 ">
					<div className="hover:rotate-180 text-2xl transition-transform duration-300 ">
						<VscChromeClose />
					</div>
				</div>
			</div>
